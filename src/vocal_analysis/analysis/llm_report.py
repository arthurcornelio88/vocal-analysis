...
Integre observações visuais com os dados numéricos na sua análise."""
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        if plot_paths:
            f.write(f"*Análise multimodal com {len([p for p in plot_paths if p.exists()])} visualizações*\n\n")
            ...
            for plot_path in sorted(plot_paths):
                if plot_path.exists():
                    ...
                    if "_f0" in name:
                        ...
                        desc = "Análise de mecanismos M1/M2 (histograma, scatter, boxplot, temporal)"
        ...
        f.write("```json\n")
        f.write(json.dumps({"stats": stats, "global": metadata.get("global", {})}, indent=2, ensure_ascii=False))
        f.write("\n```\n")
...
def main() -> None:
    ...
    parser.add_argument("--metadata", type=Path, default=Path("data/processed/ademilde_metadata.json"))
